...
public class EnviarMensaje extends Activity {
	...
	public void enviarMensaje(View v) {
		...
		if (asunto.equals("")) {
			Toast.makeText(this, "El campo asunto no puede estar vacío",
					Toast.LENGTH_LONG).show();
		} else {
			EditText etMensaje = (EditText) findViewById(R.id.etMensaje);
			mensaje = etMensaje.getText().toString();
			if (mensaje.equals("")) {
				Toast.makeText(this, "El cuerpo del mensaje no puede estar vacío",
						Toast.LENGTH_LONG).show();
			} else {
				ThreadEnviarMensaje tem = new ThreadEnviarMensaje(
						usuarioEmisor, asunto, mensaje, usuarioReceptor);
				tem.start();
				try {
					tem.join();
